try {
  //Declaracion de arreglo de elementos
  const articlesRain = [
    (article1 = {
      name: "paraguas",
      weight: 3,
  ...
  const articlesCount = (articles) => {
    articles.forEach((article) => {
      alert(
        `Este es uno de los articulos que puedes llevar dado el clima: ${article.name}`
      );
    });
    return articles;
  };

  //Logica general peso y tamanio disponible
  const addArticles = (option, aux, weightBag, weight, spaceBag) => {
    while (option && weightBag < weight && spaceBag >= 0) {
      let article = parseInt(
        prompt("Escoge el numero del indice del aticulo que deseas agregar")
      );
  ...
  //Funcion para acceder a la logica del archivo
  const fun = (aux) => {
    console.table(aux);
    ...
    console.log("La lista d elos elementos agregados son:");
    console.table(listArticles);
  };

  //Funcion para evaluar si agrega o resta mediante un boolean y operador terneario
  ...
  let climate = parseInt(
    prompt(
      "si el cima es soleado ingresa: 0\nsi el clima es con lluvia ingresa: 1 "
    )
  );
  ...
  let weight = parseInt(
    prompt("Ingresa cuanto el es peso permitido para la maleta?")
  );
  //Evaluacion de parametros definidos
